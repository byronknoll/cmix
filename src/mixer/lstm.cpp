...
Lstm::Lstm(unsigned int input_size, unsigned int num_cells,
    ...
    horizon_(horizon), input_size_(input_size) {
  hidden_[hidden_.size() - 1] = 1;
  for (int epoch = 0; epoch < horizon; ++epoch) {
    layer_input_[epoch][0].resize(257 + num_cells + input_size);
...
void Lstm::SetInput(int index, float val) {
  for (unsigned int i = 0; i < layers_.size(); ++i) {
    layer_input_[epoch_][i][256 + index] = val;
  }
}

std::valarray<float>& Lstm::Perceive(unsigned char input) {
...
std::valarray<float>& Lstm::Predict(unsigned char input) {
  for (unsigned int i = 0; i < layers_.size(); ++i) {
